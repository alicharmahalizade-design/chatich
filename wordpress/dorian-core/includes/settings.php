...
class Dorian_Settings {
    ...
    public static function sanitize($in) {
        ...
        foreach (array('work_start','work_end','currency','sms_apikey','sms_sender','sms_base','sms_mode','pay_mode',
    ...
    public static function page() {
        ...
        <table class="form-table" role="presentation">
          ...
          <tr><th>درصد بیعانه</th><td><input type="number" min="0" max="100" name="dorian_settings[deposit_rate]" value="<?php echo esc_attr($s['deposit_rate']); ?>"> ٪</td></tr>
        ...
        <h2>پنل مستقل منشی</h2>
        <table class="form-table" role="presentation">
          <tr><th>آدرس پنل منشی</th><td>
            <input type="text" name="dorian_settings[manage_slug]" value="<?php echo esc_attr($s['manage_slug']); ?>" placeholder="monshi">
            <?php if (!empty($s['manage_slug'])) echo ' <code>' . esc_html(home_url('/' . $s['manage_slug'])) . '</code>'; ?>
            <p class="description">اسلاگ لاتین. این پنل مستقل، نوبت‌های همهٔ متخصص‌ها، ارزیابی ۳۰روزه و تعرفهٔ اختصاصی خدمات را در اختیار منشی می‌گذارد.</p></td></tr>
          <tr><th>رمز ورود منشی</th><td>
            <input type="text" name="dorian_settings[manage_pass]" value="<?php echo esc_attr($s['manage_pass']); ?>">
            <p class="description">این رمز فقط برای پنل منشی است. تا وقتی خالی باشد، پنل منشی غیرفعال است.</p></td></tr>
        </table>
    ...
    public static function bookings_page() {
        ...
        echo '<div class="wrap"><h1>نوبت‌های ثبت‌شده</h1><table class="widefat striped"><thead><tr>
            <th>#</th><th>مشتری</th><th>موبایل</th><th>خدمات</th><th>زمان</th><th>مدت</th><th>تکرار</th><th>مبلغ</th><th>بیعانه</th><th>وضعیت</th></tr></thead><tbody>';
        if (!$rows) echo '<tr><td colspan="10">هنوز نوبتی ثبت نشده.</td></tr>';
        foreach ($rows as $r) {
            echo '<tr><td>' . $r->id . '</td><td>' . esc_html($r->customer_name) . '</td><td>' . esc_html($r->customer_phone) . '</td>'
                ...
                . '<td>' . number_format($r->total_price) . '</td><td>' . number_format($r->deposit_price) . '</td><td>' . esc_html($r->status) . '</td></tr>';
        }
